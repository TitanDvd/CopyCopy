...
namespace iTuner
{
	...
	public class UsbManager : Form, IDisposable
	{
		...
		private class DriverWindow : Form,/*NativeWindow*/ IDisposable
		{
		...
		public UsbDiskCollection GetAvailableDisks ()
		{
			...
			foreach (ManagementObject drive in
				new ManagementObjectSearcher(
                    "select DeviceID, Model, SerialNumber from Win32_DiskDrive where InterfaceType='USB'").Get())
			{
				...
				if (partition != null)
				{
					...
					if (logical != null)
					{
                        ...
                        disk.Name = volume["VolumeName"].ToString();
						disk.FreeSpace = (ulong)volume["FreeSpace"];
						disk.Capacity = (ulong)volume["Size"];
                        disk.VolumeSerial = (string)drive["SerialNumber"];
		...
		private void DoStateChanged (UsbStateChangedEventArgs e)
		{
			if (handler != null)
			{
				...
				if ((e.State == UsbStateChange.Added) && (e.Disk.Letter[0] != '?'))
				{
